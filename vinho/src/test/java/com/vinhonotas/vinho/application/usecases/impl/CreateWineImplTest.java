...
class CreateWineImplTest {
    ...
    void testCreateWineSuccess() {
        ...
        Mockito.when(wineRepository.createWine(Mockito.any(WineInputDTO.class))).thenReturn(wineDomain);
    ...
    void testCreateWithException() {
        ...
        Mockito.when(wineRepository.createWine(wine)).thenThrow(new BadRequestException(MessagesConstants.ERROR_CREATE_WINE));
        ...
        Mockito.verify(wineRepository, Mockito.times(1)).createWine(wine);
    }
}